...
/**
 * Controller for HTML (Non-Synon Program): WorkWithOrders (WWCONHDR).
 *
 * @author KAMALN
 */
app.controller('WorkWithOrdersController',
	function($log, $modal, $q, $rootScope, $route, $scope,
		WorkWithOrdersService, Auth, BusinessException, CallStack,
	...
	$scope.preInit = function() {
		if (!this.programName) {
			...
			this.programName = 'WorkWithOrders';
	...
	/**
	 * Entry point function invoked from (@link WorkWithOrdersEntryPanel.html)
	...
	$scope.init = function() {
		...
		var initializeOperation = WorkWithOrdersService.initialize.get({
		...
		initializeOperationPromise.then((function(response) {
			$log.info("WorkWithOrders (WWCONHDR) - Logic initialize executed.");
			...
			if (this.nextProgram.length != 0) {
				...
				$q.when(loadOperationPromise).then((function() {
					...
					else if ("WorkWithOrdersEntryPanel" !== this.nextFunction) {
	...
	$scope.loadFlatData = function() {
		...
		if (!this.pgmCalled || (this.stateVariable.zreload === "Y") || multiSelProcess) {
			if ("WorkWithOrdersEntryPanel" === method) {
				// Calls the Controller logic entryPanelDisplay method to load the
				// screen information
				this.currentOperation = WorkWithOrdersService.entryPanelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise.then((function(response) {
					$log.info("WorkWithOrders (WWCONHDR) - Logic entryPanelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			else if ("WorkWithOrdersPanel" === method) {
				// Calls the Controller logic panelDisplay method to load the
				// screen information
				this.currentOperation = WorkWithOrdersService.panelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("WorkWithOrders (WWCONHDR) - Logic panelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			else if ("WorkWithOrdersPanel3" === method) {
				// Calls the Controller logic panel3Display method to load the
				// screen information
				this.currentOperation = WorkWithOrdersService.panel3Display.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("WorkWithOrders (WWCONHDR) - Logic panel3Display executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			else if ("WorkWithOrdersPanel4" === method) {
				// Calls the Controller logic panel4Display method to load the
				// screen information
				this.currentOperation = WorkWithOrdersService.panel4Display.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("WorkWithOrders (WWCONHDR) - Logic panel4Display executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			...
			if ("WorkWithOrdersPanel".equalsIgnoreCase(method)) {
				// Calls the Controller logic panelProcess method to
				// process the screen information
				this.currentOperation = WorkWithOrdersService.panelProcess.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("WorkWithOrders (WWCONHDR) - Logic panelProcess executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			else if ("WorkWithOrdersPanel4".equalsIgnoreCase(method)) {
				// Calls the Controller logic panel4Process method to
				// process the screen information
				this.currentOperation = WorkWithOrdersService.panel4Process.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("WorkWithOrders (WWCONHDR) - Logic panel4Process executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
	...
	$scope.submitRecord = function() {
		...
		if ("WorkWithOrdersPanel" === currentScreen) {
			this.currentOperation = WorkWithOrdersService.panelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
		...
		else if ("WorkWithOrdersPanel4" === currentScreen) {
			this.currentOperation = WorkWithOrdersService.panel4Process.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
	...
	$scope.process = function(dssel, funKeyMode) {
		...
		if (funKeyMode) {
			...
			// filtering
			this.currentOperation = WorkWithOrdersService.entryPanelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("WorkWithOrders (WWCONHDR) - Logic entryPanelProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
		else {
			...
			// screen actions
			this.currentOperation = WorkWithOrdersService.gridProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("WorkWithOrders (WWCONHDR) - Logic gridProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
	...
	$scope.processQuestionMark = function() {
		...
		method = Utils.getLongMethodName(method + "p", "WWCONHDR");

		this.currentOperation = WorkWithOrdersService[method].get({
			stateVariable: this.stateVariable,
			action: this.action,
			nextFunction: this.nextFunction,
			nextProgram: this.nextProgram
		...
		currentOperationPromise.then((function(response) {
			$log.info("WorkWithOrders (WWCONHDR) - Logic executed.");
			this.assignPgmResponse(response);
	...
	$scope.manageCallStack = function(screen) {
		if (screen.startsWith("WorkWithOrders")) {
	 ...
	 * Add on grid.
	 */
	$scope.addRecordWWCONHDR01G = function() {
	 ...
	 */
	$scope.changeRecordWWCONHDR01G = function() {
		var wwconhdrg = null;
		...
		if (selectedRowList.length == 1) {
			wwconhdrg = selectedRowList[0];
		}
		else {
			wwconhdrg = this.multipleRowProcess(wwconhdrg, selectedRowList, "changeRecordWWCONHDR01G");
		}

		Utils.assignObject(this.stateVariable, wwconhdrg);
	 ...
	 */
	$scope.deleteRecordWWCONHDR01G = function() {
		var wwconhdrg = null;
		var selectedRowList = this.getSelectedRows();
		...
		if (selectedRowList.length == 1) {
			...
			wwconhdrg = this.multipleRowProcess(wwconhdrg, selectedRowList, "deleteRecordWWCONHDR01G");
		}
	 ...
	 */
	$scope.displayWWCONHDR01G = function() {
		var wwconhdrg = null;
		var selectedRowList = this.getSelectedRows();
		...
		if (selectedRowList.length == 1) {
			...
			wwconhdrg = this.multipleRowProcess(wwconhdrg, selectedRowList, "displayWWCONHDR01G");
		}
	...
	/**
	 * Lines on selected grid record.
	 */
	$scope.linesWWCONHDR01G = function() {
		var wwconhdrg = null;
		var selectedRowList = this.getSelectedRows();
		...
		if (selectedRowList.length == 1) {
			...
			wwconhdrg = this.multipleRowProcess(wwconhdrg, selectedRowList, "linesWWCONHDR01G");
		}

		Utils.assignObject(this.stateVariable, wwconhdrg);
		this.process("7", false);
	};

	/**
	 * Detail lines on flat record.
	 */
	$scope.detailLinesWWCONHDR02D = function() {
		this.stateVariable.qin = StringUtils.setIndicator(this.stateVariable.qin, 7, '1');
		this.submitRecord().then((function() {
			this.stateVariable.qin = StringUtils.setIndicator(this.stateVariable.qin, 7, '0');
		}).bind($scope));
	};
	 ...
	 */
	$scope.confirmWWCONHDR02D = function() {
	 ...
	 * DETAIL LINES on flat record.
	 */
	$scope.detailLinesWWCONHDR04D = function() {
		this.stateVariable.qin = StringUtils.setIndicator(this.stateVariable.qin, 7, '1');
		this.submitRecord().then((function() {
			this.stateVariable.qin = StringUtils.setIndicator(this.stateVariable.qin, 7, '0');
		}).bind($scope));
	...
	$scope.getNavigateTo = function(screenToGo, callFrmSubmit) {
		if (callFrmSubmit && this.isConfirmationScreen()) {
			Params[Constants.SCREEN_MODE] = Constants.CONFIRMATION_MODE;
				...
				("*EXIT" === screenToGo)) {
			while (screenToGo.startsWith("WorkWithOrders") ||
					("*EXIT" === screenToGo)) {
				screenToGo = this.cancel(this.stateVariable, "WorkWithOrders", false);
			}
	...
	$scope.applyFilter = function() {
		...
		var wwconhdrg = this.filterData;

		this.wwconhdrgXwordn = wwconhdrg.xwordn;
		this.stateVariable.locate = wwconhdrg.xwordn;
		this.fldsFiltered["Locate"] = (this.stateVariable.xwordn != null);

		this.filterApplied = (this.wwconhdrgXwordn != null);
	 ...
	 *
	 * @param wwconhdrg grid data model
	 ...
	 */
	$scope.multipleRowProcess = function(wwconhdrg, selectedRowList, callingFunName) {
		var pendingList = Params["WorkWithOrdersController_pendingRowList"];
		...
		if (selectedRowList.length != 0) {
			...
			wwconhdrg = selectedRowList.shift();
		}

		if (selectedRowList.length != 0) {
			Params["WorkWithOrdersController_pendingScreensFunction"] = callingFunName;
			pendingList = $.extend(true, [], selectedRowList);
			Params["WorkWithOrdersController_pendingRowList"] = pendingList;
		}
		else {
			Params["WorkWithOrdersController_pendingScreensFunction"] = null;
			Params["WorkWithOrdersController_pendingRowList"] = null;
		}
	...
	$scope.cancelPress = function() {
		...
		var screen = this.cancel(this.stateVariable, "WorkWithOrders", false);
