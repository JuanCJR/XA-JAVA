...
/**
 * Controller for HTML (Non-Synon Program): CustomerGroupSelection (CUSGRSEL).
 *
 * @author KAMALN
 */
app.controller('CustomerGroupSelectionController',
	function($log, $modal, $q, $rootScope, $route, $scope,
		CustomerGroupSelectionService, Auth, BusinessException, CallStack,
	...
	$scope.preInit = function() {
		if (!this.programName) {
			...
			this.programName = 'CustomerGroupSelection';
	...
	/**
	 * Entry point function invoked from (@link CustomerGroupSelectionEntryPanel.html)
	...
	$scope.init = function() {
		...
		var initializeOperation = CustomerGroupSelectionService.initialize.get({
		...
		initializeOperationPromise.then((function(response) {
			$log.info("CustomerGroupSelection (CUSGRSEL) - Logic initialize executed.");
			...
			if (this.nextProgram.length != 0) {
				...
				$q.when(loadOperationPromise).then((function() {
					...
					else if ("CustomerGroupSelectionEntryPanel" !== this.nextFunction) {
	...
	$scope.loadFlatData = function() {
		...
		if (!this.pgmCalled || (this.stateVariable.zreload === "Y")) {
			if ("CustomerGroupSelectionEntryPanel" === method) {
				// Calls the Controller logic entryPanelDisplay method to load the
				// screen information
				this.currentOperation = CustomerGroupSelectionService.entryPanelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise.then((function(response) {
					$log.info("CustomerGroupSelection (CUSGRSEL) - Logic entryPanelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			else if ("CustomerGroupSelectionPanel" === method) {
				// Calls the Controller logic panelDisplay method to load the
				// screen information
				this.currentOperation = CustomerGroupSelectionService.panelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("CustomerGroupSelection (CUSGRSEL) - Logic panelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			...
			if ("CustomerGroupSelectionPanel".equalsIgnoreCase(method)) {
				// Calls the Controller logic panelProcess method to
				// process the screen information
				this.currentOperation = CustomerGroupSelectionService.panelProcess.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("CustomerGroupSelection (CUSGRSEL) - Logic panelProcess executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
	...
	$scope.submitRecord = function() {
		...
		if ("CustomerGroupSelectionPanel" === currentScreen) {
			this.currentOperation = CustomerGroupSelectionService.panelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
	...
	$scope.process = function(dssel, funKeyMode) {
		...
		if (funKeyMode) {
			...
			// filtering
			this.currentOperation = CustomerGroupSelectionService.entryPanelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("CustomerGroupSelection (CUSGRSEL) - Logic entryPanelProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
		else {
			...
			// screen actions
			this.currentOperation = CustomerGroupSelectionService.gridProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("CustomerGroupSelection (CUSGRSEL) - Logic gridProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
	...
	$scope.manageCallStack = function(screen) {
		if (screen.startsWith("CustomerGroupSelection")) {
			this.screen = screen;
		}
	};
	...
	$scope.promptSelect = function() {
		...
		if (selectedRows.length == 0) {
			...
			$modalInstance.close([selectedRows[0], ['xwbncd', 'xwkhtx']]);
		}
	};
	...
	$scope.getNavigateTo = function(screenToGo, callFrmSubmit) {
		if ((callFrmSubmit && (this.previousFunction === this.nextFunction)) ||
				("*EXIT" === screenToGo)) {
			while (screenToGo.startsWith("CustomerGroupSelection") ||
					("*EXIT" === screenToGo)) {
				screenToGo = this.cancel(this.stateVariable, "CustomerGroupSelection", true);
			}
	...
	$scope.collectReturnValues = function() {
		...
		var rtnValList = [
			this.stateVariable.xwbncd, this.stateVariable.xwkhtx
		];
	...
	$scope.applyFilter = function() {
		...
		var cusgrselg = this.filterData;

		this.cusgrselgXwbncd = cusgrselg.xwbncd;
		this.stateVariable.xwbncd = cusgrselg.xwbncd;
		this.fldsFiltered["Xwbncd"] = (this.stateVariable.xwbncd != null);
	...
	$scope.setScreenEntryParameters = function(entryParamList) {
		...
		if (obj != null) {
			this.stateVariable.xwbncd = obj.toString();
		}

		obj = (entryParamList.length > 1) ? entryParamList[1] : null;

		if (obj != null) {
			this.stateVariable.xwkhtx = obj.toString();
		}
	};
	...
	$scope.cancelPress = function() {
		...
		var screen = this.cancel(this.stateVariable, "CustomerGroupSelection", true);
