...
/**
 * Controller for HTML (Non-Synon Program): OrderStatusSelection (ORDSTSEL).
 *
 * @author KAMALN
 */
app.controller('OrderStatusSelectionController',
	function($log, $modal, $q, $rootScope, $route, $scope,
		OrderStatusSelectionService, Auth, BusinessException, CallStack,
	...
	$scope.preInit = function() {
		if (!this.programName) {
			...
			this.programName = 'OrderStatusSelection';
	...
	/**
	 * Entry point function invoked from (@link OrderStatusSelectionEntryPanel.html)
	...
	$scope.init = function() {
		...
		var initializeOperation = OrderStatusSelectionService.initialize.get({
		...
		initializeOperationPromise.then((function(response) {
			$log.info("OrderStatusSelection (ORDSTSEL) - Logic initialize executed.");
			...
			if (this.nextProgram.length != 0) {
				...
				$q.when(loadOperationPromise).then((function() {
					...
					else if ("OrderStatusSelectionEntryPanel" !== this.nextFunction) {
	...
	$scope.loadFlatData = function() {
		...
		if (!this.pgmCalled || (this.stateVariable.zreload === "Y")) {
			if ("OrderStatusSelectionEntryPanel" === method) {
				// Calls the Controller logic entryPanelDisplay method to load the
				// screen information
				this.currentOperation = OrderStatusSelectionService.entryPanelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise.then((function(response) {
					$log.info("OrderStatusSelection (ORDSTSEL) - Logic entryPanelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			else if ("OrderStatusSelectionPanel" === method) {
				// Calls the Controller logic panelDisplay method to load the
				// screen information
				this.currentOperation = OrderStatusSelectionService.panelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("OrderStatusSelection (ORDSTSEL) - Logic panelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			...
			if ("OrderStatusSelectionPanel".equalsIgnoreCase(method)) {
				// Calls the Controller logic panelProcess method to
				// process the screen information
				this.currentOperation = OrderStatusSelectionService.panelProcess.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("OrderStatusSelection (ORDSTSEL) - Logic panelProcess executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
	...
	$scope.submitRecord = function() {
		...
		if ("OrderStatusSelectionPanel" === currentScreen) {
			this.currentOperation = OrderStatusSelectionService.panelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
	...
	$scope.process = function(dssel, funKeyMode) {
		...
		if (funKeyMode) {
			...
			// filtering
			this.currentOperation = OrderStatusSelectionService.entryPanelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("OrderStatusSelection (ORDSTSEL) - Logic entryPanelProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
		else {
			...
			// screen actions
			this.currentOperation = OrderStatusSelectionService.gridProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("OrderStatusSelection (ORDSTSEL) - Logic gridProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
	...
	$scope.manageCallStack = function(screen) {
		if (screen.startsWith("OrderStatusSelection")) {
			this.screen = screen;
		}
	};
	...
	$scope.promptSelect = function() {
		...
		if (selectedRows.length == 0) {
			...
			$modalInstance.close([selectedRows[0], ['xwstat', 'xwsdsc']]);
		}
	};
	...
	$scope.getNavigateTo = function(screenToGo, callFrmSubmit) {
		if ((callFrmSubmit && (this.previousFunction === this.nextFunction)) ||
				("*EXIT" === screenToGo)) {
			while (screenToGo.startsWith("OrderStatusSelection") ||
					("*EXIT" === screenToGo)) {
				screenToGo = this.cancel(this.stateVariable, "OrderStatusSelection", true);
			}
	...
	$scope.collectReturnValues = function() {
		...
		var rtnValList = [
			this.stateVariable.xwstat, this.stateVariable.xwsdsc
		];
	...
	$scope.applyFilter = function() {
		...
		var ordstselg = this.filterData;

		this.ordstselgXwstat = ordstselg.xwstat;
		this.stateVariable.xwstat = ordstselg.xwstat;
		this.fldsFiltered["Xwstat"] = (this.stateVariable.xwstat != null);
	...
	$scope.setScreenEntryParameters = function(entryParamList) {
		...
		if (obj != null) {
			this.stateVariable.xwstat = obj.toString();
		}

		obj = (entryParamList.length > 1) ? entryParamList[1] : null;

		if (obj != null) {
			this.stateVariable.xwsdsc = obj.toString();
		}
	};
	...
	$scope.cancelPress = function() {
		...
		var screen = this.cancel(this.stateVariable, "OrderStatusSelection", true);
