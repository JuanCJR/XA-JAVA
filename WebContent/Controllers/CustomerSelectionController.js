...
/**
 * Controller for HTML (Non-Synon Program): CustomerSelection (CUSTSSEL).
 *
 * @author KAMALN
 */
app.controller('CustomerSelectionController',
	function($log, $modal, $q, $rootScope, $route, $scope,
		CustomerSelectionService, Auth, BusinessException, CallStack,
	...
	$scope.preInit = function() {
		if (!this.programName) {
			...
			this.programName = 'CustomerSelection';
	...
	/**
	 * Entry point function invoked from (@link CustomerSelectionEntryPanel.html)
	...
	$scope.init = function() {
		...
		var initializeOperation = CustomerSelectionService.initialize.get({
		...
		initializeOperationPromise.then((function(response) {
			$log.info("CustomerSelection (CUSTSSEL) - Logic initialize executed.");
			...
			if (this.nextProgram.length != 0) {
				...
				$q.when(loadOperationPromise).then((function() {
					...
					else if ("CustomerSelectionEntryPanel" !== this.nextFunction) {
	...
	$scope.loadFlatData = function() {
		...
		if (!this.pgmCalled || (this.stateVariable.zreload === "Y")) {
			if ("CustomerSelectionEntryPanel" === method) {
				// Calls the Controller logic entryPanelDisplay method to load the
				// screen information
				this.currentOperation = CustomerSelectionService.entryPanelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise.then((function(response) {
					$log.info("CustomerSelection (CUSTSSEL) - Logic entryPanelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			else if ("CustomerSelectionPanel" === method) {
				// Calls the Controller logic panelDisplay method to load the
				// screen information
				this.currentOperation = CustomerSelectionService.panelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("CustomerSelection (CUSTSSEL) - Logic panelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			...
			if ("CustomerSelectionPanel".equalsIgnoreCase(method)) {
				// Calls the Controller logic panelProcess method to
				// process the screen information
				this.currentOperation = CustomerSelectionService.panelProcess.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("CustomerSelection (CUSTSSEL) - Logic panelProcess executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
	...
	$scope.submitRecord = function() {
		...
		if ("CustomerSelectionPanel" === currentScreen) {
			this.currentOperation = CustomerSelectionService.panelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
	...
	$scope.process = function(dssel, funKeyMode) {
		...
		if (funKeyMode) {
			...
			// filtering
			this.currentOperation = CustomerSelectionService.entryPanelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("CustomerSelection (CUSTSSEL) - Logic entryPanelProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
		else {
			...
			// screen actions
			this.currentOperation = CustomerSelectionService.gridProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("CustomerSelection (CUSTSSEL) - Logic gridProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
	...
	$scope.manageCallStack = function(screen) {
		if (screen.startsWith("CustomerSelection")) {
			this.screen = screen;
		}
	};
	...
	$scope.promptSelect = function() {
		...
		if (selectedRows.length == 0) {
			...
			$modalInstance.close([selectedRows[0], ['xwbccd', 'xwg4tx']]);
		}
	};
	...
	$scope.getNavigateTo = function(screenToGo, callFrmSubmit) {
		if ((callFrmSubmit && (this.previousFunction === this.nextFunction)) ||
				("*EXIT" === screenToGo)) {
			while (screenToGo.startsWith("CustomerSelection") ||
					("*EXIT" === screenToGo)) {
				screenToGo = this.cancel(this.stateVariable, "CustomerSelection", true);
			}
	...
	$scope.collectReturnValues = function() {
		...
		var rtnValList = [
			this.stateVariable.xwbccd, this.stateVariable.xwg4tx
		];
	...
	$scope.applyFilter = function() {
		...
		var custsselg = this.filterData;

		this.custsselgXwbccd = custsselg.xwbccd;
		this.stateVariable.xwbccd = custsselg.xwbccd;
		this.fldsFiltered["Xwbccd"] = (this.stateVariable.xwbccd != null);
	...
	$scope.cancelPress = function() {
		...
		var screen = this.cancel(this.stateVariable, "CustomerSelection", true);
