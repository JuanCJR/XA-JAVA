...
/**
 * Controller for HTML (Non-Synon Program): ProductMasterSelection (STKMASEL).
 *
 * @author KAMALN
 */
app.controller('ProductMasterSelectionController',
	function($log, $modal, $q, $rootScope, $route, $scope,
		ProductMasterSelectionService, Auth, BusinessException, CallStack,
	...
	$scope.preInit = function() {
		if (!this.programName) {
			...
			this.programName = 'ProductMasterSelection';
	...
	/**
	 * Entry point function invoked from (@link ProductMasterSelectionEntryPanel.html)
	...
	$scope.init = function() {
		...
		var initializeOperation = ProductMasterSelectionService.initialize.get({
		...
		initializeOperationPromise.then((function(response) {
			$log.info("ProductMasterSelection (STKMASEL) - Logic initialize executed.");
			...
			if (this.nextProgram.length != 0) {
				...
				$q.when(loadOperationPromise).then((function() {
					...
					else if ("ProductMasterSelectionEntryPanel" !== this.nextFunction) {
	...
	$scope.loadFlatData = function() {
		...
		if (!this.pgmCalled || (this.stateVariable.zreload === "Y")) {
			if ("ProductMasterSelectionEntryPanel" === method) {
				// Calls the Controller logic entryPanelDisplay method to load the
				// screen information
				this.currentOperation = ProductMasterSelectionService.entryPanelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise.then((function(response) {
					$log.info("ProductMasterSelection (STKMASEL) - Logic entryPanelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			else if ("ProductMasterSelectionPanel" === method) {
				// Calls the Controller logic panelDisplay method to load the
				// screen information
				this.currentOperation = ProductMasterSelectionService.panelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("ProductMasterSelection (STKMASEL) - Logic panelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			...
			if ("ProductMasterSelectionPanel".equalsIgnoreCase(method)) {
				// Calls the Controller logic panelProcess method to
				// process the screen information
				this.currentOperation = ProductMasterSelectionService.panelProcess.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("ProductMasterSelection (STKMASEL) - Logic panelProcess executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
	...
	$scope.submitRecord = function() {
		...
		if ("ProductMasterSelectionPanel" === currentScreen) {
			this.currentOperation = ProductMasterSelectionService.panelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
	...
	$scope.process = function(dssel, funKeyMode) {
		...
		if (funKeyMode) {
			...
			// filtering
			this.currentOperation = ProductMasterSelectionService.entryPanelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("ProductMasterSelection (STKMASEL) - Logic entryPanelProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
		else {
			...
			// screen actions
			this.currentOperation = ProductMasterSelectionService.gridProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("ProductMasterSelection (STKMASEL) - Logic gridProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
	...
	$scope.manageCallStack = function(screen) {
		if (screen.startsWith("ProductMasterSelection")) {
			this.screen = screen;
		}
	};
	...
	$scope.promptSelect = function() {
		...
		if (selectedRows.length == 0) {
			...
			$modalInstance.close([selectedRows[0], ['xwabcd', 'xwhltx']]);
		}
	};
	...
	$scope.getNavigateTo = function(screenToGo, callFrmSubmit) {
		if ((callFrmSubmit && (this.previousFunction === this.nextFunction)) ||
				("*EXIT" === screenToGo)) {
			while (screenToGo.startsWith("ProductMasterSelection") ||
					("*EXIT" === screenToGo)) {
				screenToGo = this.cancel(this.stateVariable, "ProductMasterSelection", true);
			}
	...
	$scope.collectReturnValues = function() {
		...
		var rtnValList = [
			this.stateVariable.xwabcd, this.stateVariable.xwhltx
		];
	...
	$scope.applyFilter = function() {
		...
		var stkmaselg = this.filterData;

		this.stkmaselgXwabcd = stkmaselg.xwabcd;
		this.stateVariable.xwabcd = stkmaselg.xwabcd;
	...
	$scope.setScreenEntryParameters = function(entryParamList) {
		...
		if (obj != null) {
			this.stateVariable.xwabcd = obj.toString();
		}

		obj = (entryParamList.length > 1) ? entryParamList[1] : null;

		if (obj != null) {
			this.stateVariable.xwhltx = obj.toString();
		}
	};
	...
	$scope.cancelPress = function() {
		...
		var screen = this.cancel(this.stateVariable, "ProductMasterSelection", true);
