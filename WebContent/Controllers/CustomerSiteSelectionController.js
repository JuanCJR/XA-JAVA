...
/**
 * Controller for HTML (Non-Synon Program): CustomerSiteSelection (CUSFSEL).
 *
 * @author KAMALN
 */
app.controller('CustomerSiteSelectionController',
	function($log, $modal, $q, $rootScope, $route, $scope,
		CustomerSiteSelectionService, Auth, BusinessException, CallStack,
	...
	$scope.preInit = function() {
		if (!this.programName) {
			...
			this.programName = 'CustomerSiteSelection';
	...
	/**
	 * Entry point function invoked from (@link CustomerSiteSelectionEntryPanel.html)
	...
	$scope.init = function() {
		...
		var initializeOperation = CustomerSiteSelectionService.initialize.get({
		...
		initializeOperationPromise.then((function(response) {
			$log.info("CustomerSiteSelection (CUSFSEL) - Logic initialize executed.");
			...
			if (this.nextProgram.length != 0) {
				...
				$q.when(loadOperationPromise).then((function() {
					...
					else if ("CustomerSiteSelectionEntryPanel" !== this.nextFunction) {
	...
	$scope.loadFlatData = function() {
		...
		if (!this.pgmCalled || (this.stateVariable.zreload === "Y")) {
			if ("CustomerSiteSelectionEntryPanel" === method) {
				// Calls the Controller logic entryPanelDisplay method to load the
				// screen information
				this.currentOperation = CustomerSiteSelectionService.entryPanelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise.then((function(response) {
					$log.info("CustomerSiteSelection (CUSFSEL) - Logic entryPanelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			else if ("CustomerSiteSelectionPanel" === method) {
				// Calls the Controller logic panelDisplay method to load the
				// screen information
				this.currentOperation = CustomerSiteSelectionService.panelDisplay.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("CustomerSiteSelection (CUSFSEL) - Logic panelDisplay executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
			...
			if ("CustomerSiteSelectionPanel".equalsIgnoreCase(method)) {
				// Calls the Controller logic panelProcess method to
				// process the screen information
				this.currentOperation = CustomerSiteSelectionService.panelProcess.get({
					stateVariable: this.stateVariable,
					action: this.action,
					nextFunction: this.nextFunction,
					nextProgram: this.nextProgram
				...
				currentOperationPromise = this.currentOperation.$promise;
				currentOperationPromise.then((function(response) {
					$log.info("CustomerSiteSelection (CUSFSEL) - Logic panelProcess executed.");
					this.assignPgmResponse(response);
				}).bind($scope), this.handleError.bind($scope));
			}
	...
	$scope.submitRecord = function() {
		...
		if ("CustomerSiteSelectionPanel" === currentScreen) {
			this.currentOperation = CustomerSiteSelectionService.panelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
	...
	$scope.process = function(dssel, funKeyMode) {
		...
		if (funKeyMode) {
			...
			// filtering
			this.currentOperation = CustomerSiteSelectionService.entryPanelProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("CustomerSiteSelection (CUSFSEL) - Logic entryPanelProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
		else {
			...
			// screen actions
			this.currentOperation = CustomerSiteSelectionService.gridProcess.get({
				stateVariable: this.stateVariable,
				action: this.action,
				nextFunction: this.nextFunction,
				nextProgram: this.nextProgram
			...
			currentOperationPromise = this.currentOperation.$promise;
			currentOperationPromise.then((function(response) {
				$log.info("CustomerSiteSelection (CUSFSEL) - Logic gridProcess executed.");
				this.assignPgmResponse(response);
			}).bind($scope), this.handleError.bind($scope));
		}
	...
	$scope.manageCallStack = function(screen) {
		if (screen.startsWith("CustomerSiteSelection")) {
			this.screen = screen;
		}
	};
	...
	$scope.promptSelect = function() {
		...
		if (selectedRows.length == 0) {
			...
			$modalInstance.close([selectedRows[0], ['cusno']]);
		}
	};
	...
	$scope.getNavigateTo = function(screenToGo, callFrmSubmit) {
		if ((callFrmSubmit && (this.previousFunction === this.nextFunction)) ||
				("*EXIT" === screenToGo)) {
			while (screenToGo.startsWith("CustomerSiteSelection") ||
					("*EXIT" === screenToGo)) {
				screenToGo = this.cancel(this.stateVariable, "CustomerSiteSelection", true);
			}
	...
	$scope.applyFilter = function() {
		...
		var cusfselg = this.filterData;

		this.cusfselgDscusno = cusfselg.dscusno;
		this.stateVariable.cusno = cusfselg.dscusno;
		this.fldsFiltered["Cusno"] = (this.stateVariable.dscusno != null);
	...
	$scope.setScreenEntryParameters = function(entryParamList) {
		...
		if (obj != null) {
			this.stateVariable.cusno = parseInt(obj, 10);
		}
	};
	...
	$scope.cancelPress = function() {
		...
		var screen = this.cancel(this.stateVariable, "CustomerSiteSelection", true);
