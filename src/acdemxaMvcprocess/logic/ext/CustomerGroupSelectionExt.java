...
import acdemxaMvcprocess.logic.data.CustomerGroupSelectionGDO;
import acdemxaMvcprocess.logic.data.CustomerGroupSelectionPDO;
...
/**
 * Program logic for Customer group Selection (CUSGRSELRA).
 *
 * @author KAMALN
 */
public class CustomerGroupSelectionExt {

	/**
	 * Initialises the <code>CustomerGroupSelectionPDO</code>
	 * object to receive the data to display.
	 */
	public void pgmInitialize() {
		zzsetdspfind();
		stateVariable.setPcode(stateVariable.getXwbncd());
		stateVariable.setParnam(stateVariable.getXwkhtx());
	...
	private void zbuild() {
		...
		while (! nmfkpinds.pgmInd31()) {
			...
			gridVariable[uwScnIdx1].setXwbncd(stateVariable.getXwbncd());
			gridVariable[uwScnIdx1].setXwkhtx(stateVariable.getXwkhtx());
	...
	public CustomerGroupSelectionPDO stateVariable = new CustomerGroupSelectionPDO();
	public DataCRUD<CustomerGroups, String> customerGroups = new DataCRUD<CustomerGroups, String>(CustomerGroups.class, this, new String[] { "xwbncd" }, new char[] { 'A' }, new char[] { 'R' });
	@SuppressWarnings("unused")
	private Integer msgObjIdx = 0;
	private Integer uwScnIdx1 = 0;
	private String zzNxtFun = "";
	public CustomerGroupSelectionGDO gridVariable[] = initArray(CustomerGroupSelectionGDO.class, uwRow1 + 1);
