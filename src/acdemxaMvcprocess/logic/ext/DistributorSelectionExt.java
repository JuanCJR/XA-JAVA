...
/**
 * Program logic for Distributor Selection (DISTSSELRA).
 *
 * @author KAMALN
 */
public class DistributorSelectionExt {

	/**
	 * Initialises the <code>DistributorSelectionPDO</code>
	 * object to receive the data to display.
	 */
	public void pgmInitialize() {
		zzsetdspfind();
		stateVariable.setPcode(stateVariable.getDsdcde());
		stateVariable.setParnam(stateVariable.getDname());
	...
	private void zbuild() {
		...
		while (! nmfkpinds.pgmInd31()) {
			...
			gridVariable[uwScnIdx1].setDsdcde(stateVariable.getDsdcde());
			gridVariable[uwScnIdx1].setDname(stateVariable.getDname());
	...
	public DistributorSelectionPDO stateVariable = new DistributorSelectionPDO();
	public DataCRUD<Distributors, String> distributors = new DataCRUD<Distributors, String>(Distributors.class, this, new String[] { "dsdcde" }, new char[] { 'A' }, new char[] { 'R' });
	@SuppressWarnings("unused")
	private Integer msgObjIdx = 0;
	private Integer uwScnIdx1 = 0;
	private String zzNxtFun = "";
	public DistributorSelectionGDO gridVariable[] = initArray(DistributorSelectionGDO.class, uwRow1 + 1);
